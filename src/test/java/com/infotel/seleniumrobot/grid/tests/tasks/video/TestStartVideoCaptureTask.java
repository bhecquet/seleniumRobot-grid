...
public class TestStartVideoCaptureTask extends BaseMockitoTest {
	...
	public void testStartVideo() throws Exception {
		try (MockedStatic mockedCustomWebDriver = Mockito.mockStatic(CustomEventFiringWebDriver.class)) {
			mockedCustomWebDriver.when(() -> CustomEventFiringWebDriver.startVideoCapture(any(DriverMode.class), isNull(), any(File.class), anyString())).thenReturn(recorder);

			new StartVideoCaptureTask("1234").execute();

			mockedCustomWebDriver.verify(() -> CustomEventFiringWebDriver.startVideoCapture(DriverMode.LOCAL, null, Paths.get(Utils.getRootdir(), VideoCaptureTask.VIDEOS_FOLDER).toFile(), "1234.avi"));
