...
/**
 * Compares two sets of {@link Capabilities} against a desired standard.
 * Capabilities are compared by...
 * <ol>
 ...
 * </ol>
 * For all comparisons, if the capability is missing, that particular criteria
 * shall not factor into the comparison.
 *
 * <p>
 * When comparing platforms, preference will be given to an exact platform match
 * over a fuzzy match (e.g. Platform.WINDOWS will match Platform.WINDOWS before
 * it matches Platform.XP). Furthermore, configurations matching the current
 * system's platform will be given preference. For example, when
 * {@code Platform.getCurrent() == Platform.WINDOWS}, a set of Capabilities with
 * {@code Platform.WINDOWS} will score higher than one with {@code Platform.ANY}
 * .
 */
public class CustomCapabilitiesComparator implements Comparator<Capabilities> {
	...
	@VisibleForTesting
	static <T extends Capabilities> T getBestMatch(Capabilities against, Collection<T> toCompare,
			Platform currentPlatform) {
		return Ordering.from(new CustomCapabilitiesComparator(against, currentPlatform)).max(toCompare);
	}
